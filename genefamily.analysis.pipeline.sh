...
do
    ...
    ?)
        ...
        echo "Usage: $(basename $0) [-i path][-s site]"
        exit 1
        ;;
    esac
done
...
python2 ~/TOOLS/cafe_tutorial/python_scripts/cafetutorial_prep_r8s.py -i r8s.tmp -o r8s.input -s $site_num -p $pairs -c $time
echo "Obtain the r8s ultra tree"
tree=`/home/panda2bat/TOOLS/r8s1.81/src/r8s -b -f r8s.input|tail -n 1|awk '{split($0, a, " = "); sub("Sapmus", "", a[2]); print a[2]}'`
...
awk 'BEGIN{FS=OFS="\t"}{print "Desc", $0}' Orthogroups.GeneCount.tsv > cafe.input
...
~/TOOLS/CAFE/bin/cafe cafe.run.sh
python2 ~/TOOLS/cafe_tutorial/python_scripts/cafetutorial_report_analysis.py -i cafe.result.cafe -o cafe.summary
echo -e "\033[32m========================finish run cafe=================================\033[0m"

echo -e "\033[32m===========================enrichment of cafe family====================\033[0m"
awk 'BEGIN{FS="\t"}{if($0~/IaIo<[0-9]+>:/){split($2, a, ","); for(i in a){if(a[i]~"\[+[0-9]+\*\]"){print a[i]}}}}' cafe.summary_fams.txt|awk '{split($0,a,"["); print a[1]}'> Ia-io.Expansions.family
awk 'BEGIN{FS="\t"}{if($0~/IaIo<[0-9]+>:/){split($2, a, ","); for(i in a){if(a[i]~"\[-[0-9]+\*\]"){print a[i]}}}}' cafe.summary_fams.txt|awk '{split($0,a,"["); print a[1]}'> Ia-io.Contraction.family
